...
class tetramino:
	...
	def taille(self):
		""" Fonction qui calcul la largueur et la longueur maximale du tétramino"""
		xmin=99
		xmax=0
		ymin=99
		ymax=0
	...
	# def tombe(self):
	# 	""" Fais descendre le tetramino d'une case """
...
class Grille:
	...
	def tomber(self,tetramino):
		...
		for x,y in tetramino.pos:
		    if self.plateau[x,y+1]>0:
		        OK = False
		    pos_chute.append((x,y+1))

		if OK == True:
		    for x,y in pos_chute:
		        self.plateau[x,y-1] = 0
		    for x,y in pos_chute: 
		        self.plateau[x,y] = tetramino.couleur
		    
		    tetramino.pos = pos_chute
		else :
			print("OUCH LE MUR")

	def tourne(self,tetramino):
		pos_tourne = []
