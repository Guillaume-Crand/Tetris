...
size = 30
tetraminos=[[(1,1),(1,2),(2,1),(2,2)],[(1,1),(2,1),(2,2),(3,2)],[(1,2),(2,1),(2,2),(3,1)],[(1,1),(2,1),(3,1),(4,1)],[(1,1),(1,2),(1,3),(2,2)],[(1,1),(1,2),(2,2),(3,2)],[(1,1),(1,2),(2,1),(3,1)]]
...
color_fantome 	= {		  1:redfantome,2:yellowfantome,3:bluefantome,4:greenfantome,5:purplefantome,6:turquoisefantome,7:orangefantome}
...
class Grille:
	def __init__(self,screen,petit=False,position=(50,50),marge=30, taille_next=6, numero = 1, malus = 0, reseau = False):
		if petit==False:
			self.taille = (10,20)
		...
		self.next_position  = (self.position[0]+size*(self.taille[0]+2)+self.marge*2 , self.position[1])
	...
	def zonedejeu(self,pos,taille):
		...
		pygame.draw.rect(screen, (255,255,255), (pos[0]-marge, pos[1]-marge, taille[0]+(2*marge),taille[1]+(2*marge) ))
	...
	def tourne(self,tetramino,sens_horaire=False):
		...
		if not sens_horaire :
			...
			for (x,y) in tetramino.pos:
				pos_tourne.append(( xmin + ((L-1) - (y-ymin)) + dec , ymin + ((x-xmin) )))
	...
	def decale(self,debut,fin = 0 ,decalage = 1,sens = -1):
		...
		for ny in range (debut,fin,sens):
			...
			for nx in range(self.taille[0] , 0, -1):
				...
				carre(self.screen,(nx,ny+decalage),self.position,color[abs(self.plateau[nx,ny])])
				(self.plateau[nx,ny+decalage] , self.plateau[nx,ny]) = (self.plateau[nx,ny] , 0)
	...
	def test_ligne(self):
		...
		if nb_ligne > 0:
			...
			pygame.event.post(pygame.event.Event(LIGNE,{"joueur":self.numero,"nombre":nb_ligne}))
	...
	def test_malus(self):
		...
		if self.malus>0:
			self.decale(1+self.malus,21,-self.malus,1)	#On ne prends pas 1 pour ne pas dessiner en 0
			...
			pygame.event.post(pygame.event.Event(J2,{"touche":"malus","trou":trou,"malus":self.malus}))
			self.malus = 0
	...
	def draw_next(self):
		...
		### On met un fond noir
		(p1,p2)  = self.next_position
		pygame.draw.rect(self.screen,(0,0,0),(p1,p2,self.next_taille*size,self.next_taille*size))
	...
	def pos_tetra_fantome(self):
		...
		"""
		copie = self.tetra_bouge.pos.copy() # on réalise une copie des positions du tétramino dont on veut le fantome
		limite = [] # cette liste sert à tester si la place est libre
		for i in range(len(copie)): # on parcour les position du tétramino
			limite.append((copie[i][0],copie[i][1]+1)) # on ajoute chaque position à la liste limite
		while self.place_libre(limite): # on test grace à limite si la place est libre un rang en dessous du tétramino
			...
			for i in range(len(copie)):
				limite.append((copie[i][0],copie[i][1]+1)) # on remplace les positions de limite par de nouvelles positions( un range en dessous des anciennes)
		return copie
	...
	def draw_tetra_fantome(self):
		...
		self.tetra_fantome.affiche(avant,apres,hors=self.tetra_bouge.pos,fantome = True)
