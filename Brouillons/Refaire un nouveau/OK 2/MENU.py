...
class Menu:
	def __init__(self):
		...
		self.fenetre 		= pygame.display.set_mode((self.hauteur_ecran,self.longueur_ecran))
		self.police 		= pygame.font.Font("Polices/ArbutusSlab.ttf",20,bold="False",italic="False")
		self.couleurs 		= {"rouge": (255,0,0), "vert":(0,255,0), "bleu":(0,0,255),"gris": (86,86,86), "blanc":(255,255,255)}

	def parametres_menu(self):
		""" initialisation et placement de l'image de fond et du logo """
		# Fond
		self.fond 		= pygame.image.load("images/TetrisFond.png")
		self.fenetre.blit(self.fond,(0,0))

		# Logo
		self.image_menu = pygame.image.load("images/TetrisLogo.png")
		self.fenetre.blit(self.image_menu,(self.hauteur_ecran/3.45,self.longueur_ecran/7))

		#Nom de la fenêtre
		pygame.display.set_caption("Tetris - G/V/M/T") 
...
class Musique:
	def __init__(self):
		...
		self.playlist_musique = list()
		self.playlist_musique.append("Musique/TetrisBase.wav")

		#Chargement musique
		pygame.mixer.music.load(self.playlist_musique.pop()) 
		pygame.mixer.music.play(-1) #Lancement musique
		
		# BOUCLE QUI NE MARCHE PAS POUR METTRE EN PAUSE LA MUSIQUE
		continuer = 1
...
class Bouton:
 def __init__(self,texte,position,fonction,couleur = (86,86,86)):
    self.texte      = texte
    self.position   = position
...
self.couleurs   = {"rouge": (255,0,0), "vert":(0,255,0), "bleu":(0,0,255),"gris": (86,86,86), "blanc":(255,255,255)}
self.police     = pygame.font.Font("Polices/ArbutusSlab.ttf",20,bold="False",italic="False")
self.boutons    = [] #liste vide de boutons

def fen_1(self):
self.boutons.append(Bouton("Jouer",(200, 200, 270, 50),self.fen_2)) #330, 225
...
class MenuFrame:
    ...
    def __init__(self, background):
        self.buttons.append(AButton(Menu(),"Options", (200,300,270,50), self.couleurs["gris"], self.options, 330, 325)) #création et ajout du bouton dans la liste
        self.buttons.append(AButton(Menu(),"Quitter", (200,400,270,50), self.couleurs["gris"], self.quitter, 330, 425)) #création et ajout du bouton dans la liste
    
    #Fonction qui est active lorsque le bouton "Jouer" est cliqué
    def jouer(self):
        print("Jouer")
        return SecondFrame(self.background) #Appel Classe de la seconde fenêtre "Fenetre de jeu"
    
    #Fonction qui est active lorsque le bouton "Options" est cliqué
    def options(self):
        print("Options")
        return ThirdFrame(self.background) #Appel Classe de la 3e fenêtre "Fenêtre des options"
...
class SecondFrame:
    ...
    def __init__(self, background):
        self.background = background #fond
        self.couleurs={"rouge": (255,0,0), "vert":(0,255,0), "bleu":(0,0,255), 
                       "gris": (86,86,86), "blanc":(255,255,255)} #Dico Couleurs
        self.buttons = [] #Liste vide de boutons
        self.buttons.append(AButton(Menu(),"Solo", (90, 230, 200, 50), self.couleurs["gris"], self.solo, 195,255)) #Création et ajout du bouton dans la liste
        self.buttons.append(AButton(Menu(),"Multijoueur en Réseau", (350,230,200,50), self.couleurs["gris"], self.multireseau, 450,255)) #Création et ajout du bouton dans la liste
        self.buttons.append(AButton(Menu(),"Multijoueur Local", (90,350,200,50), self.couleurs["gris"], self.multilocal, 195,375)) #Création et ajout du bouton dans la liste
        self.buttons.append(AButton(Menu(),"Retour", (350,350,200,50), self.couleurs["gris"], self.retour, 450,375)) #Création et ajout du bouton dans la liste
    ...
    def multireseau(self):
        ...
        img = pygame.image.load("images/TetrisFinMulti.png")
...
class ThirdFrame:
    ...
    def __init__(self, background):
        ...
        self.buttons = [] #Liste vide de boutons
        self.buttons.append(AButton(Menu(),"Fullscreen", (90, 230, 200, 50), self.couleurs["gris"], self.fullscreen, 195,255)) #Création et ajout du bouton dans la liste
        self.buttons.append(AButton(Menu(),"Credit", (350,230,200,50),self.couleurs["gris"], self.credit, 450,255)) #Création et ajout du bouton dans la liste
        self.buttons.append(AButton(Menu(),"Scores", (90,350,200,50),self.couleurs["gris"], self.scores, 195,375)) #Création et ajout du bouton dans la liste
        self.buttons.append(AButton(Menu(),"Retour", (350,350,200,50),self.couleurs["gris"], self.retour, 450,375)) #Création et ajout du bouton dans la liste
    ...
    def credit(self):
        return CreditFrame(self.background) #Appel de la classe qui génère la fenêtre des crédits
    ...
    def retour(self):
        return MenuFrame(self.background) #Rappel de la classe générant la 1ere fenêtre
...
class CreditFrame:
    ...
    def __init__(self,background):
        self.background = background #fond
        self.couleurs={"rouge": (153,0,0), "Or": (39,174,96), "bleu":(0,0,255), 
                       "gris": (86,86,86), "blanc":(255,255,255), "noir":(0,0,0)} #Dico couleurs
        self.buttons = [] #Liste vide de boutons
    ...
    def retour(self):
        return ThirdFrame(self.background) #Rappel de la classe générant la fenêtre des options
...
class ScoreFrame:
    ...
    def __init__(self,background,score):
        ...
        self.background = background #fond
        self.couleurs={"rouge": (200,0,0), "bleuté": (102,102,199), "bleu":(153,0,255), 
                       "gris": (86,86,86), "blanc":(255,255,255), "noir":(0,0,0), "text":(100,100,153)} #Dico couleurs
        self.buttons = [] #Liste vide de boutons
    ...
    def retour(self):
        return ThirdFrame(self.background) #Rappel de la classe qui génère la fenêtre des options
...
class Quit:
    ...
    def __init__(self,background,menu):
        ...
        self.menu=menu
        self.buttons = [] #Liste vide de boutons
        self.img= pygame.image.load("images/TetrisQuit.png")
...
if __name__ == "__main__":
    ...
    menu = MenuFrame("images/TetrisFond.png")
    musique = Musique()
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN: #Lors du click
                for button in menu.buttons: #dans la liste de boutons
                    if Rect(button.position).collidepoint(event.pos): #vérification de la coordonnée
                        menu = button.func() #Appel de la fonction

            if event.type == QUIT:  #Si on quitte la fenêtre (sans passer par les boutons)
                sys.exit() #Quitte le programme

        
        for button in menu.buttons: 
            pygame.draw.rect(screen.fenetre, button.couleur, button.position) #Dessine le bouton
            font=pygame.font.Font("Polices/ArbutusSlab.ttf",16,bold="False",italic="False") #Charge la police d'écriture
            txt=font.render(button.texte, True, (255,255,255)) #Ecriture dans le bouton du texte prédéfini
            pos_texte=txt.get_rect(center=(button.x,button.y)) #Selon les coordonnées prédéfinis
