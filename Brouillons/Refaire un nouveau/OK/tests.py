		...
		while continuer == 1:
			...
			for event in pygame.event.get():
				...
				if event.type == KEYDOWN:
					if event.key == K_LEFT: # Aller a droite		
						j1.deplacer(j1.tetra_bouge,-1)

					if event.key == K_RIGHT:# Aller à gauche
						j1.deplacer(j1.tetra_bouge,1)

					if event.key == K_UP:# Faire tourner le tetramino
						j1.tourne(j1.tetra_bouge, False)

					if event.key == K_DOWN:	# Faire tomber le tetramino très vite 
