...
class Grille:
	def __init__(self,screen,petit=False,position=(10,10),marge=30, taille_next=6, numero = 1, malus = 0):
		if petit==False:
			self.taille = (10,20)
	...
	def arret(self,tetramino):
		...
		- test si la ligne est complete, si le joueur a perdu et s'il a un malus
		...
		pygame.time.set_timer(USEREVENT + self.numero,600)
	...
	def centrer(self,tetramino):
		dec = int(self.taille[0]/2 - int(tetramino.taille()[3]/2) )
		newposition=[]
		for (x,y) in tetramino.pos:
			newposition.append((x+dec, y))

		self.replace(tetramino,newposition)
	...
	def zonedejeu(self,pos,taille):
		...
		# a cause du décallage : il n'y a pas de case (0,0) donc +size
	...
	def draw_next(self):
		...
		### On met un fond noir
		(p1,p2)  = (self.next_position[0]+self.marge,self.next_position[1]+self.marge)
		pygame.draw.rect(self.screen,(0,0,0),(p1,p2,6*size,6*size))

		### On dessine le tetramino
		self.next.draw(self.next.pos,self.next_tetra_pos())

	def pos_tetra_fantome(self):
		copie = self.tetra_bouge.pos.copy()
		limite = []
		for i in range(len(copie)):
			limite.append((copie[i][0],copie[i][1]+1))
		while self.place_libre(limite):
	...
	def change_tetra_bouge(self,tetra):
		...
		self.replace(self.tetra_bouge.pos,tetra.pos,True)		
