...
class tetramino:
	def __init__(self,screen,marge_grille):
	 	...
	 	self.couleurs = color[- self.couleur]
	...
	def draw(self,pos,marge):
		for positions in pos:
			carre(self.screen,positions,marge,self.couleurs)
...
class Grille:
	def __init__(self,screen,petit=False,position=(10,10),marge=30, taille_next=6, numero = 1):
		if petit==False:
			self.taille = (10,20)
	...
	def test_perdu(self):
		if not self.place_libre(self.next.pos):
			print("PERDU")
			pygame.event.post(pygame.event.Event(PERDU,{}))
			#EVENT
	...
	def zonedejeu(self,pos,taille):
		...
		#print ((pos[0]-marge, pos[1]-marge, taille[0]+(2*marge),taille[1]+(2*marge)),((pos[0]		 , pos[1]	   , taille[0]			,taille[1]			  )))
