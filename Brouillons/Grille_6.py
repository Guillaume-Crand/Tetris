...
def zonedejeu(x, y, taille):
	pygame.draw.rect(screen, (255,255,255), (x-marge, y-marge, taille[0]+(2*marge),taille[1]+(2*marge)))
	pygame.draw.rect(screen, (0,0,0), (x, y, taille[0],taille[1]))

#zonedejeu(200, 20,(size*10,size*20))



def carre(pos,couleur):
...
class tetramino:
	def __init__(self):
	 	...
	 	self.pos = [(4,4),(5,4),(6,4),(7,4)]  #,,(5,4),(8,4),,(7,4),(8,4),
	 	self.couleur = -7
	 	self.couleurs = purple
			...
			for x in range (0,L):
				for y in range (0,L):
					if (self.plateau [xmin + x ,ymin + y ] == tetramino.couleur):
						pos_tourne.append(( (xmin) + (y + dec) , (ymin) + ((L-1) - x)))
