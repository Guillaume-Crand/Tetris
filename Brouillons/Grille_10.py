...
class tetramino:
	def __init__(self,screen,marge_grille):
	 	...
	 	self.screen = screen
	 	self.pos = tetraminos[randint(0,6)]
...
class Grille:
	def __init__(self,screen,petit=False,position=(10,10),marge=30, taille_next=6, numero = 1, malus = 0):
		...
		self.screen 		= screen
		self.tetraminos		= []								# peut-être un groupe de sprite
		self.numero 		= numero
		self.malus			= 0
		self.score 			= 0
	...
	def tourne(self,tetramino,sens_horaire=False):
		...
		if not sens_horaire :
			...
			for (x,y) in tetramino.pos:
				pos_tourne.append(( xmin+ ( (L-1) - (x-xmin) ) , ymin + ((y-ymin) + dec )))
	...
	def centrer(self,tetramino):
		dec = int(self.taille[0]/2 - (int(tetramino.taille()[3] /2)+1) )
		newposition=[]
		for(x,y)in tetramino.pos:
			newX= x + dec
			newposition.append((newX,y))
	...
	def next_tetra_pos(self):
		...
		newp1=int(p1+((1/2)*borgrand-(1/2)*borptilarg) -size)
		newp2=int(p2+((1/2)*borgrand-(1/2)*borptithaut)-size)
