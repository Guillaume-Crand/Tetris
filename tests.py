...
def partie(screen, type_jeu=1):
    ...
    if type_jeu == 4:
        ...
        def message_reseau(mes):
            if mes == "tetra_bouge":
                reseau_jeu.message(
                    "touche = tetra_bouge / pos = "
                    + str(j1.tetra_bouge.numpos)
                    + " / couleur = "
                    + str(j1.tetra_bouge.couleur)
                )
            elif mes == "tetra_next":
                reseau_jeu.message(
                    "touche = tetra_next / pos = "
                    + str(j1.next.numpos)
                    + " / couleur = "
                    + str(j1.next.couleur)
                )
                ...
                reseau_jeu.message(
                    "touche = apres_next / pos = "
                    + str(j1.apres_next[len(j1.apres_next) - 1].numpos)
                    + " / couleur = "
                    + str(j1.apres_next[len(j1.apres_next) - 1].couleur)
                )
        ...
        reseau_jeu.message(
            "touche = apres_next / pos = "
            + str(j1.apres_next[0].numpos)
            + " / couleur = "
            + str(j1.apres_next[0].couleur)
        )
        reseau_jeu.message(
            "touche = apres_next / pos = "
            + str(j1.apres_next[1].numpos)
            + " / couleur = "
            + str(j1.apres_next[1].couleur)
        )
            ...
            for event in pygame.event.get():
                ...
                if event.type == J2:
                    ...
                    # 	ok = 1
                        ...
                        j2.change_tetra_bouge(
                            tetramino(screen, j2.position, event.pos, event.couleur)
                        )
